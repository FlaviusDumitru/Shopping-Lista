...
try {
    ...
    // Vara är ok!
    echo "<p class = 'info'> Test vara är ok</p>";
    varaOK($ch);
...
function varaOK($curlHandle) {
            ...
            // Koppla databas och sätt möjlighet att ångra förändrngar
            $db = connectDB();
...
function varaForLangtNamn ($curlHandle) {
        ...
        // Lägg till vara med långt namn
        $data = ['vara' => 'Ett jättelångt namn med massor av bokstäver som gör att det är alldeles för långt för att få plats'];
        curl_setopt($curlHandle, CURLOPT_POSTFIELDS, $data);
